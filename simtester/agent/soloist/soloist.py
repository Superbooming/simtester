...
class SoloistAgent(Agent):
    def __init__(self, model: Optional[str] = None, model_file: Optional[os.PathLike] = None,
        ...
        if self.model_name in model_register_table:
            ...
            self._init_model()
            self._init_dataset()
            super(SoloistAgent, self).__init__(self._interact_fn, self._start_fn, name)
        else:
    ...
    def _init_config(self, model, model_file, archive_file, device, config):
        ...
        if not self.config:
            config_path = os.path.join(CONFIG_PATH, f'{self.dataset}/soloist/{self.model_name}.yaml')
    ...
    def _init_model(self):
        ...
        self.model = self.model.from_pretrained(self.model_file).to(self.device)
    ...
    def _init_tokenizer(self):
        if self.archive_file:
            ...
            self.archive_file = check_and_get_data('archive', self.dataset, 'soloist-archive')
        self.tokenizer = self.tokenizer.from_pretrained(self.archive_file)
        special_tokens_path = os.path.join(self.archive_file, 'special_tokens.txt')
        for line in open(special_tokens_path, 'r', encoding='utf-8'):
            line = line.strip('\n')
    ...
    def _init_dataset(self):
        check_and_get_data('archive', self.dataset, 'db')
        ...
        slot_value_path = os.path.join(self.archive_file, 'db_slot_value.pkl')
    ...
    def _interact_fn(self, context: List[str], is_fill: bool = True, return_bs: bool = False) -> str:
        ...
        if is_fill:
            ...
            ret = res
        ret = ret.replace(' -s', 's')
        ret = ret.replace(' -ly', 'ly')
        ret = ret.replace(' -er', 'er')
        if return_bs:
            ret = (ret, res_bs)
        return ret
