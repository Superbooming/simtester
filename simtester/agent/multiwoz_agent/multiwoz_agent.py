...
class MultiWOZAgent(Agent):
    ...
    def _init_config(self, model, model_dir, device, config):
        ...
        if config:
            ...
            assert 'model_name' in self.config['model']
            self.model_name = self.config['model']['model_name']
            assert 'model_dir' in self.config['model']
    ...
    def _init_model(self):
        ...
        if 'special_tokens' in self.config['tokenizer']:
            self.model.resize_token_embeddings(len(self.tokenizer))
        if 'eos_token' in self.config['tokenizer']:
    ...
    def _init_tokenizer(self):
        ...
        if 'special_tokens' in self.config['tokenizer']:
            special_tokens_dir = os.path.join(ROOT_PATH, self.config['tokenizer']['special_tokens'])
            for line in open(special_tokens_dir, 'r', encoding='utf-8'):
                line = line.strip('\n')
                self.tokenizer.add_tokens(line)
        if 'eos_token' in self.config['tokenizer']:
            self.tokenizer.eos_token = self.config['tokenizer']['eos_token']
    ...
    def _interact_fn(self, context: List[str]) -> str:
        ...
        def generate_sequence(model, tokenizer, context, token_type_ids, attention_mask, max_length=512, num_samples=1,
            ...
            if num_mask < 1.0:
                ...
                token_type_ids_ori[0] = token_type_ids_ori[0][-self.config['model']['max_context_length']:]
                context_ori[0] = context_ori[0][-self.config['model']['max_context_length']:]
                attention_mask_ori[0] = attention_mask_ori[0][-self.config['model']['max_context_length']:]
