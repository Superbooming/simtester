...
from simtester.agent.damd.db_ops import MultiWozDB
from simtester.agent.damd.clean_dataset import clean_text
from simtester.agent.damd.ontology import all_domains, get_slot, dialog_acts, dialog_act_params, eos_tokens
from simtester.agent.damd.utils import Vocab, padSeqs
from simtester.config.multiwoz.config import *

DEFAULT_DIRECTORY = ARCHIVE_PATH
default_path = os.path.join(ARCHIVE_PATH, 'data/multi-woz-processed')
...
class MultiWozReader(object):
    def __init__(self, config):
        ...
        self.cfg = config
        self.db = MultiWozDB(DEFAULT_DIRECTORY, self.cfg.dbs)
        self.vocab_size = self._build_vocab()
        # self.domain_files = json.loads(open(self.cfg.domain_file_path, 'r').read())
        self.slot_value_set = json.loads(open(os.path.join(DEFAULT_DIRECTORY, self.cfg.slot_value_set_path), 'r').read())
    ...
    def _build_vocab(self):
        self.vocab = Vocab(self.cfg.vocab_size)
        self.vocab.load_vocab(os.path.join(DEFAULT_DIRECTORY, self.cfg.vocab_path_eval))
    ...
    def prepare_input_np(self, u, u_delex):
        ...
        if self.first_turn:
            ...
            for item, py_list in self.py_prev.items():
                ...
                if not self.cfg.enable_bspn and 'bspn' in item:
                    continue
                prev_np = padSeqs(py_list, truncated=self.cfg.truncated, trunc_method='pre')
        ...
        for item in ['user', 'usdx']:
            ...
            inputs[item+'_np'] = padSeqs(py_list, truncated=self.cfg.truncated, trunc_method=trunc_method)
